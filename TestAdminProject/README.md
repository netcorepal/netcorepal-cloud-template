...
cd src/TestAdminProject.AppHost
dotnet run
```
...
Rider 用户可以直接使用 `TestAdminProject.sln.DotSettings` 文件中的 Live Templates 配置。
...
# 强制更新数据库
dotnet ef database update -p src/TestAdminProject.Infrastructure 

# 创建迁移 SEE：https://learn.microsoft.com/zh-cn/ef/core/managing-schemas/migrations/?tabs=dotnet-core-cli
dotnet ef migrations add InitialCreate -p src/TestAdminProject.Infrastructure 
```
