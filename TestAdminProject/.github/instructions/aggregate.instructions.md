---
applyTo: "src/TestAdminProject.Domain/AggregatesModel/**/*.cs"
---
...
- 类文件应放置在 `src/TestAdminProject.Domain/AggregatesModel/{AggregateName}Aggregate/` 目录下。
- 例如 `src/TestAdminProject.Domain/AggregatesModel/UserAggregate/User.cs`。
- 每个聚合在独立文件夹中。
- 聚合根类名与文件名一致。
- 强类型ID与聚合根定义在同一文件中。
...
文件: `src/TestAdminProject.Domain/AggregatesModel/UserAggregate/User.cs`

```csharp
using TestAdminProject.Domain.DomainEvents; // 必需：引用领域事件
