...
```
TestAdminProject.sln
├── src/
│   ├── TestAdminProject.Domain/         # 领域层 - 聚合根、实体、领域事件
│   ├── TestAdminProject.Infrastructure/ # 基础设施层 - EF配置、仓储接口、仓储实现
│   └── TestAdminProject.Web/           # 表现层 - API、应用服务
└── test/                            # 测试项目
    ├── TestAdminProject.Domain.UnitTests/         # 领域层测试项目
    ├── TestAdminProject.Infrastructure.UnitTests/ # 基础设施层测试项目
    └── TestAdminProject.Web.UnitTests/           # 表现层测试项目
```
...
### 文件组织
- **聚合根** → `src/TestAdminProject.Domain/AggregatesModel/{AggregateFolder}/`
- **领域事件** → `src/TestAdminProject.Domain/DomainEvents/`
- **仓储** → `src/TestAdminProject.Infrastructure/Repositories/`
- **实体配置** → `src/TestAdminProject.Infrastructure/EntityConfigurations/`
- **命令与命令处理器** → `src/TestAdminProject.Web/Application/Commands/`
- **查询与查询处理器** → `src/TestAdminProject.Web/Application/Queries/`
- **API端点** → `src/TestAdminProject.Web/Endpoints/`
- **领域事件处理器** → `src/TestAdminProject.Web/Application/DomainEventHandlers/`
- **集成事件** → `src/TestAdminProject.Web/Application/IntegrationEvents/`
- **集成事件转换器** → `src/TestAdminProject.Web/Application/IntegrationEventConverters/`
- **集成事件处理器** → `src/TestAdminProject.Web/Application/IntegrationEventHandlers/`
...
**Web层** (`src/TestAdminProject.Web/GlobalUsings.cs`):
...
**Infrastructure层** (`src/TestAdminProject.Infrastructure/GlobalUsings.cs`):
...
**Domain层** (`src/TestAdminProject.Domain/GlobalUsings.cs`):
...
**查询处理器**:
```csharp
using TestAdminProject.Domain.AggregatesModel.{AggregateFolder};
...
```csharp
using TestAdminProject.Domain.AggregatesModel.{AggregateFolder};
using TestAdminProject.Web.Application.Commands.{FeatureFolder};
using TestAdminProject.Web.Application.Queries.{FeatureFolder};
```
